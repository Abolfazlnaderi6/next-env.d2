...
export default function WaterSavingPage() {
  ...
  const [dishPerDay, setDishPerDay] = useState(0);
  const [toiletFlushesPerDay, setToiletFlushesPerDay] = useState(0);
  const [gardenPerWeek, setGardenPerWeek] = useState(0);
  const [carWashPerMonth, setCarWashPerMonth] = useState(0);
  ...
  const calculate = () => {
    ...
    const waterPerDish = 20; // لیتر هر بار شستن ظرف
    const waterPerToilet = 6; // لیتر هر بار فلاش
    const waterPerGarden = 100; // لیتر هر بار آبیاری
    const waterPerCarWash = 200; // لیتر هر بار شستشوی ماشین
    const dailyDish = dishPerDay * waterPerDish;
    const dailyToilet = toiletFlushesPerDay * waterPerToilet;
    const dailyGarden = (gardenPerWeek * waterPerGarden) / 7;
    const dailyCarWash = (carWashPerMonth * waterPerCarWash) / 30;
        ...
        <div className="bg-white rounded-3xl p-8 lg:p-12 border border-slate-200 shadow-xl">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700">تعداد دفعات شستن ظرف در روز</label>
                <input type="number" value={dishPerDay} onChange={(e) => setDishPerDay(Number(e.target.value))} className="w-full px-4 py-3 border rounded-lg" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700">تعداد دفعات استفاده از توالت در روز</label>
                <input type="number" value={toiletFlushesPerDay} onChange={(e) => setToiletFlushesPerDay(Number(e.target.value))} className="w-full px-4 py-3 border rounded-lg" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700">تعداد دفعات آبیاری باغچه در هفته</label>
                <input type="number" value={gardenPerWeek} onChange={(e) => setGardenPerWeek(Number(e.target.value))} className="w-full px-4 py-3 border rounded-lg" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700">تعداد دفعات شستشوی ماشین در ماه</label>
                <input type="number" value={carWashPerMonth} onChange={(e) => setCarWashPerMonth(Number(e.target.value))} className="w-full px-4 py-3 border rounded-lg" />
              </div>
